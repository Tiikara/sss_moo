...
pub trait Solution<Buffer>: Debug + Clone {
    /// Perform a crossover operation with another solution candidate
    fn crossover(&mut self, buffer: &mut Buffer, other: &mut Self);

    /// Mutate a solution candidate
    fn mutate(&mut self, buffer: &mut Buffer);
}

/// An optimization objective trait
pub trait Objective<Buffer, S: Solution<Buffer>> {
...
pub trait Constraint<Buffer, S: Solution<Buffer>> {
...
pub trait Meta<'a, Buffer, S: Solution<Buffer>> {
    ...
    /// Cannot be empty
    fn objectives(&self) -> &Vec<Box<dyn Objective<Buffer, S> + 'a>>;

    /// Return an optional list of optimization constraints
    fn constraints(&self) -> &Vec<Box<dyn Constraint<Buffer, S> + 'a>>;
}

pub trait SolutionsRuntimeProcessor<Buffer, S: Solution<Buffer>> {
    fn initialize_new_candidates(&mut self, candidates: Vec<&mut S>);
    ...
    fn create_solutions_memory_buffer(&mut self) -> Box<dyn MemoryTypedBuffer<S>>;
}
